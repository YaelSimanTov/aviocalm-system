...
const AddTherapistForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const usernameRegex = /^(?=.*[0-9]).{5,}$/;
    if (!usernameRegex.test(form.username)) {
      setError("Username must be at least 5 characters and include a digit.");
      return;
    }
  ...
  // 🔥 תוספת 3: פונקציה שסוגרת את המודאל ומעבירה דף
  const handleCloseModal = () => {
    setIsModalOpen(false);
    navigate('/admin/therapists'); // מעביר אותך לרשימת המטפלים
  };
